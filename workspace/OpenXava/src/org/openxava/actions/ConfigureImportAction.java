...
public class ConfigureImportAction extends TabBaseAction
	...
	private void fillDataSampleLine(Scanner scanner, List<ImportColumn> columns, int lineNumber) {
		...
		StringTokenizer st = new StringTokenizer(line, XavaPreferences.getInstance().getCSVSeparator());
		int count = Math.min(st.countTokens(), columns.size());
		for (int i=0; i<count; i++) {
			String field = Strings.unquote(st.nextToken());
