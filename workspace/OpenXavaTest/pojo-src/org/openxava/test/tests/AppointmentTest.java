...
public class AppointmentTest extends ModuleTestBase {
	...
	private void assertImport(String file) throws Exception {
		...
		assertValueInCollection("columns", 2, 0, "");
		assertValueInCollection("columns", 2, 1, "Start time"); 
		assertValueInCollection("columns", 2, 2, "9/11/17 11:45 AM"); 
		assertValueInCollection("columns", 2, 3, "9/12/17 4:15 PM");
		
		assertValueInCollection("columns", 3, 0, "Description");
		assertValueInCollection("columns", 3, 1, "Descrip"); 
		assertValueInCollection("columns", 3, 2, "Meeting with my friend");  
		assertValueInCollection("columns", 3, 3, "");
		...
		setValueInCollection("columns", 2, "nameInApp", "time");
		
		execute("Import.import");
		assertErrorsCount(1); 
		assertError("Error importing {time=2017-09-12 16:15:00.0}: Value for description in appointment is required"); 
